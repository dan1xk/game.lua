...
local dy = 0
local bullets = {}
local shootTimer = global.SHOOT_INTERVAL
...
function drawPlayer()
	...
	love.graphics.circle("fill", player.x, player.y, global.PLAYER_SIZE)
...
function drawPlayerCannon()
	...
	local offsetX = global.PLAYER_SIZE / 2
	local offsetY = -15
...
function game.update(dt)
	local delta = movement(dt, player, global.PLAYER_SPEED, playerSize, w, h)
	...
	if isMouseDown then
		...
		if shootTimer <= 0 then
			...
			shootTimer = global.SHOOT_INTERVAL
		end
...
function updateBullets(dt)
	for i = #bullets, 1, -1 do
		...
		bullet.x = bullet.x + bullet.dx * global.BULLET_SPEED * dt
		bullet.y = bullet.y + bullet.dy * global.BULLET_SPEED * dt
